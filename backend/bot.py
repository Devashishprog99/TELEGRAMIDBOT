...
def get_main_menu(is_admin=False):
    ...
    if is_admin:
        ...
        # Row 5: Broadcast & Settings
        builder.row(
            InlineKeyboardButton(text="📢 Broadcast", callback_data="btn_broadcast"),
            InlineKeyboardButton(text="⚙️ Bot Settings", callback_data="btn_bot_settings")
        )
...
async def show_bot_settings_menu(callback: types.CallbackQuery):
    """Show bot settings configuration menu (Admin only)"""
    admin_id = int(os.getenv("ADMIN_TELEGRAM_ID", "0"))
    ...
    async with async_session() as session:
        ...
        current_channel = chan_setting.value if (chan_setting and chan_setting.value) else "Not set"
        current_owner = owner_setting.value if (owner_setting and owner_setting.value) else "Not set"
    
    text = "⚙️ <b>Bot Settings</b>\n\n"
    text += f"📢 <b>Channel Link:</b>\n<code>{current_channel}</code>\n\n"
    text += f"👤 <b>Owner Username:</b>\n<code>{current_owner}</code>\n\n"
    text += "Select what you want to configure:"
    
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="📢 Set Channel Link", callback_data="set_channel_link"))
    builder.row(InlineKeyboardButton(text="👤 Set Owner Username", callback_data="set_owner_username"))
    builder.row(InlineKeyboardButton(text="🏠 Main Menu", callback_data="btn_main_menu"))
...
async def set_channel_link_start(callback: types.CallbackQuery, state: FSMContext):
    """Start channel link configuration"""
    admin_id = int(os.getenv("ADMIN_TELEGRAM_ID", "0"))
    ...
    text = "📢 <b>Set Channel Link</b>\n\n"
    text += "Please send the channel link (e.g., <code>https://t.me/yourchannel</code>)\n\n"
    text += "⚠️ Make sure the link is correct - this will be used for force join!"
    
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="❌ Cancel", callback_data="btn_bot_settings"))
...
async def process_channel_link(message: types.Message, state: FSMContext):
    ...
    text = f"✅ <b>Channel Link Updated!</b>\n\n"
    text += f"📢 <b>New Channel Link:</b>\n<code>{channel_link}</code>\n\n"
    text += "Users will now be asked to join this channel before using the bot."
    
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="⚙️ Bot Settings", callback_data="btn_bot_settings"))
    builder.row(InlineKeyboardButton(text="🏠 Main Menu", callback_data="btn_main_menu"))
...
async def set_owner_username_start(callback: types.CallbackQuery, state: FSMContext):
    """Start owner username configuration"""
    admin_id = int(os.getenv("ADMIN_TELEGRAM_ID", "0"))
    ...
    text = "👤 <b>Set Owner Username</b>\n\n"
    text += "Please send the owner username (must start with @)\n\n"
    text += "Example: <code>@yourusername</code>\n\n"
    text += "This will be displayed in the Support menu."
...
async def process_owner_username(message: types.Message, state: FSMContext):
    ...
    text = f"✅ <b>Owner Username Updated!</b>\n\n"
    text += f"👤 <b>New Owner Username:</b> {username}\n\n"
    text += "This will now be displayed when users click the Support button."
